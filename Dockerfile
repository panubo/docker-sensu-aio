...
# Some dependencies
RUN apt-get update && \
  apt-get -y install curl sudo bc inotify-tools apt-transport-https && \
  curl -L https://github.com/just-containers/skaware/releases/download/v1.20.0/s6-2.5.1.0-linux-amd64-bin.tar.gz | tar -C / -zxf - && \
  apt-get clean && \
...
ENV UCHIWA_VERSION 0.25.3
ENV UCHIWA_PKG_VERSION 1

# Setup sensu package repo & Install Sensu
RUN curl https://sensu.global.ssl.fastly.net/apt/pubkey.gpg | apt-key add - && \
